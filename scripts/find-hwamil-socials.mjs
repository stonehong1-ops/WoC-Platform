...
async function findSocials() {
  ...
  snap.docs.forEach(d => {
    ...
    if (venueId === '4E7HtYTd66dimCLw9NV4' || title.includes('fire') || titleNative.includes('화밀') || titleNative.includes('탱고파이어') || titleNative.includes('화요')) {
  ...
  if (!found) {
    console.log('No Hwamil social found.');
  }
}

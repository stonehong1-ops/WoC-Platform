...
async function findLumali() {
  ...
  snap.docs.forEach(d => {
    ...
    if (title.includes('lumali') || titleNative.includes('루말리') || (city.includes('daejeon') && venue.includes('onada'))) {
  ...
  if (!found) {
    console.log('No Lumali or Daejeon Onada social found.');
  }
}

findLumali().then(() => process.exit(0)).catch(err => {
  console.error(err);
  process.exit(1);
});

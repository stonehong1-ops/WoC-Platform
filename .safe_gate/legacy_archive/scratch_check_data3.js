...
async function main() {
  console.log("=== CHECKING CLASSES (collectionGroup) ===");
  const classesSnap = await getDocs(collectionGroup(db, 'classes'));
  console.log("Total classes found in all subcollections:", classesSnap.size);
  classesSnap.forEach(d => {
    const data = d.data();
    const pathSegments = d.ref.path.split('/');
    const groupId = pathSegments[1] || '';
    if (groupId === 'freestyle-tango') {
      console.log("CLASS FOUND:", d.id, "title:", data.title, "groupId:", groupId, "schedule:", JSON.stringify(data.schedule));
    }
  });

  console.log("\n=== CHECKING SOCIALS (collection) ===");
  const socialsSnap = await getDocs(collection(db, 'socials'));
  socialsSnap.forEach(d => {
    const data = d.data();
    const isMatched = data.venueId === '2mvxZZVNWzJ4MwDIAWq3' || data.organizerId === 'freestyle-tango';
    if (isMatched) {

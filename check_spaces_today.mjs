...
async function main() {
  const snap = await db.collection("rental_spaces").get();
  console.log("TOTAL SPACES IN DB:", snap.size);
  ...
  snap.docs.forEach(d => {
    ...
    if ((created && created >= todayStart) || (updated && updated >= todayStart)) {
      console.log(`- Space ID: ${d.id} | Title: ${data.title} | GroupId: ${data.groupId} | Location: ${data.location} | Created: ${created} | Updated: ${updated}`);
    }
  });
}

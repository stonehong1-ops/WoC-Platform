...
async function updateCategory(names: string[], newCategory: string) {
  for (const name of names) {
    ...
    // Need to handle both "name" and "nameKo" possibly or just "name".
    // Try exact match on 'name'
    const q = query(collection(db, 'venues'), where('name', '==', name));
    const querySnapshot = await getDocs(q);
    
    if (querySnapshot.empty) {
      console.log(`  -> Not found: ${name}. Will try a looser search if needed.`);
    } else {
...
async function main() {
  try {
    console.log('--- Updating to Club ---');
    await updateCategory(toClub, 'Club');
    
    console.log('\n--- Updating to Stay ---');
    await updateCategory(toStay, 'Stay');

  ...
  return (
    ...
    >
      {img ? (
        ...
        <div className="w-full h-full flex items-center justify-center bg-amber-500/10 text-amber-600 text-[10px] font-black uppercase">
          {name.charAt(0)}
        </div>
      )}
    </div>
    ...
    <div className="w-full max-w-[340px] bg-white/95 border border-zinc-100 rounded-3xl p-5 shadow-md relative overflow-hidden transition-all duration-300 hover:shadow-lg my-2 dark:bg-zinc-900/90 dark:border-zinc-800 animate-in zoom-in-95 duration-200">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          <span className={`material-symbols-outlined text-[20px] ${isConfirmed ? 'text-green-500' : 'text-amber-600'}`}>
      ...
      <div className="mb-4">
        ...
        {description && (
          <p className="text-[12px] font-medium text-zinc-500 line-clamp-2 leading-relaxed text-left">
            {description}
          </p>
        )}
      </div>
      ...
      {attendees.length > 0 && (
        <div className="mb-5 text-left">
          <div className="text-[10px] font-black text-zinc-400 uppercase tracking-wider mb-2.5">
      ...
      <div className="flex flex-col gap-2">
        ...
        {/* Confirmed Banner */}
        {isConfirmed && (
          <div className="w-full py-3 bg-green-50/50 border border-green-100 rounded-2xl text-[12px] font-bold text-green-600 flex items-center justify-center gap-1.5 animate-in zoom-in-95 duration-200">
            <span className="material-symbols-outlined text-[16px]">verified</span>
            {t('chat.meetup_confirmed_desc', '일정이 확정되어 개인 일정표에 등록되었습니다.')}
          </div>
        )}
      </div>
    </div>

...
export default function ClassDetail({ groupId, onClose, isOpen, itemId, itemDetail: propItemDetail, onManage }: ClassDetailProps) {
  ...
  const targetOwnerUid = itemDetail?.authorId || groupDetails?.ownerId || 'admin';
  ...
  const handleBlockToggle = async () => {
    ...
    try {
      if (isBlocked) {
        ...
        try {
          const autoReportId = `${user.uid}_block_${targetOwnerUid}`;
    ...
    <div className="fixed inset-0 z-[9999] bg-white flex flex-col antialiased animate-in slide-in-from-bottom duration-300">
      ...
      {itemDetail && (
        <ReportModal
          ...
          targetId={itemDetail.id}
          targetType="post"
          targetTitle={itemDetail.title}
          targetOwnerUid={targetOwnerUid}
          targetSnapshot={itemDetail.description || itemDetail.title || 'No snapshot'}
        />
      )}
    </div>
  );

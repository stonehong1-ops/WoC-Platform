...
const COLLECTION_NAME = 'music365';
...
export default function Music365Popup({ onClose }: Music365PopupProps) {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() } as MusicEntry));
      setEntries(all);
      if (all.length > 0) setCurrentIndex(0);
      else setCurrentIndex(-1);
    } catch (error) {
      console.error("[Music365] fetch error:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Listen to reviews
  useEffect(() => {
    if (!currentEntry) { setReviews([]); return; }
    const reviewsRef = collection(db, COLLECTION_NAME, currentEntry.id, 'reviews');
  ...
  const handleUpload = async () => {
    ...
    try {
      const q = query(collection(db, COLLECTION_NAME), orderBy('createdAt', 'desc'));
      const snap = await getDocs(q);
      const all = snap.docs.map(d => d.data() as MusicEntry);
      const nextEp = all.length > 0 && all[0].episodeNumber ? all[0].episodeNumber + 1 : 1;

      const fileRef = ref(storage, `${COLLECTION_NAME}/ep${nextEp}_${Date.now()}_${uploadFile.name}`);
      ...
      console.error("[Music365] upload error:", error);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, COLLECTION_NAME, currentEntry.id, 'reviews'), {
        content: newReview.trim(),
        createdAt: serverTimestamp()
      });
      setNewReview('');
    } catch (error) {
      console.error("[Music365] review error:", error);
    } finally {
  ...
  const toggleLike = async () => {
    ...
    const entryRef = doc(db, COLLECTION_NAME, currentEntry.id);
    ...
    } catch (error) { console.error("[Music365] like error:", error); }
  };
  ...
  useEffect(() => {
    ...
    window.history.pushState({ popup: 'music365' }, '');
  ...
  return createPortal(
    <div className="fixed inset-0 bg-gradient-to-br from-amber-950 via-stone-900 to-slate-900 flex flex-col animate-in fade-in duration-300 overflow-hidden" style={{ zIndex: 99999 }}>

      {/* Decorative Background Pattern */}
      <div className="absolute inset-0 opacity-5" style={{ backgroundImage: "url('https://www.transparenttextures.com/patterns/music-pattern.png')" }} />

      {/* Header */}
      <div className="absolute top-0 left-0 right-0 p-5 flex justify-between items-center z-30">
        <div className="flex flex-col">
          <span className="px-3 py-1 bg-amber-500/20 text-amber-300 text-[10px] font-bold uppercase tracking-widest rounded-full mb-1 inline-block backdrop-blur-md border border-amber-500/30 w-fit">
            🎵 Camus' Selection
          </span>
      ...
      {isAdmin && (
        <div className="absolute top-20 right-6 z-40 bg-white/95 backdrop-blur-md p-6 rounded-3xl shadow-2xl border border-slate-200 flex flex-col gap-5 w-[340px] animate-in fade-in zoom-in duration-200">
          ...
          <button onClick={handleUpload} disabled={!uploadFile || uploading} className="w-full py-3.5 bg-amber-600 text-white rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2 transition-all shadow-md active:scale-95">
            {uploading ? <div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <><span className="material-symbols-outlined text-lg">cloud_upload</span>Upload</>}
          </button>
        </div>
      )}
      ...
      <div className="flex-1 w-full h-full relative flex items-center justify-center pt-24 pb-8" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
        {loading ? (
          ...
          <>
            ...
            <div className="absolute top-20 left-1/2 -translate-x-1/2 z-10">
              <span className="text-white/60 font-black tracking-widest uppercase text-xs bg-white/10 px-4 py-2 rounded-full backdrop-blur-md border border-white/10">
                Day {currentEntry.episodeNumber} of 365
              </span>
            </div>
            ...
            <div className="relative max-w-lg w-full mx-4">
              ...
              <div className="absolute -bottom-3 -right-3 w-16 h-16 rounded-full bg-amber-500/20 backdrop-blur-md border border-amber-500/30 flex items-center justify-center">
                <span className="material-symbols-outlined text-amber-400 text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>music_note</span>
              </div>
            </div>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1 z-10">
              {entries.slice(Math.max(0, currentIndex - 3), Math.min(entries.length, currentIndex + 4)).map((_, i) => {
                ...
                return <div key={idx} className={`rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-6 h-2 bg-amber-400' : 'w-2 h-2 bg-white/30'}`} />;
              })}
            </div>
          </>
        ) : (
          <div className="flex flex-col items-center gap-4 text-white/50">
            ...
            <p className="font-bold">No music posts yet.</p>
          </div>
        )}
      </div>
      ...
      {currentEntry && !showBottomSheet && (
        <div className="absolute bottom-8 right-6 z-20 flex items-center gap-3 animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <button onClick={() => { setShowBottomSheet(true); window.history.pushState({ popup: 'music365Sheet' }, ''); }} className="relative w-14 h-14 bg-amber-600/95 backdrop-blur-md rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-105 active:scale-95 transition-transform border border-amber-500/30">
            <span className="material-symbols-outlined text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>chat_bubble</span>
            <span className="absolute -top-1.5 -right-1.5 bg-amber-500 text-white text-[11px] font-black min-w-[24px] h-6 px-1.5 flex items-center justify-center rounded-full border-2 border-amber-700 shadow-sm">{reviews.length}</span>
          </button>
        </div>
      )}
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[32px] z-40 max-h-[85vh] flex flex-col animate-in slide-in-from-bottom-full duration-300 shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
              <form onSubmit={handleSubmitReview} className="flex flex-col sm:flex-row items-end gap-3 mb-8 bg-slate-50 p-3 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <button type="submit" disabled={!newReview.trim() || submittingReview} className="w-full sm:w-auto h-[44px] px-6 bg-amber-600 text-white rounded-2xl font-bold text-sm disabled:opacity-50 disabled:bg-slate-200 disabled:text-slate-400 flex items-center justify-center transition-all active:scale-95 shrink-0">
                ...
                {reviews.length === 0 ? (
                  ...
                  <div key={review.id} className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-amber-50 flex items-center justify-center shrink-0">
                      <span className="material-symbols-outlined text-amber-400 text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>person</span>
                    </div>

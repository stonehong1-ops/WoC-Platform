...
export default function TangoHistoryPopup({ onClose }: TangoHistoryPopupProps) {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() } as HistoryEntry));
      setEntries(all);
      if (all.length > 0) setCurrentIndex(0);
      else setCurrentIndex(-1);
    } catch (error) {
      console.error("[TangoHistory] fetch error:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const q = query(collection(db, COLLECTION_NAME), orderBy('createdAt', 'desc'));
      const snap = await getDocs(q);
      const all = snap.docs.map(d => d.data() as HistoryEntry);
      ...
      console.error("[TangoHistory] upload error:", error);
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("[TangoHistory] review error:", error);
    } finally {
  ...
  const toggleLike = async () => {
    ...
    } catch (error) { console.error("[TangoHistory] like error:", error); }
  };
  ...
  useEffect(() => {
    ...
    window.history.pushState({ popup: 'tangoHistory' }, '');
  ...
  return createPortal(
    <div className="fixed inset-0 bg-gradient-to-br from-indigo-950 via-slate-900 to-stone-900 flex flex-col animate-in fade-in duration-300 overflow-hidden" style={{ zIndex: 99999 }}>
      ...
      {isAdmin && (
        <div className="absolute top-20 right-6 z-40 bg-white/95 backdrop-blur-md p-6 rounded-3xl shadow-2xl border border-slate-200 flex flex-col gap-5 w-[340px] animate-in fade-in zoom-in duration-200">
          ...
          <button onClick={handleUpload} disabled={!uploadFile || uploading} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2 transition-all shadow-md active:scale-95">
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="relative max-w-lg w-full mx-4">
              <img src={currentEntry.imageUrl} alt={`Chapter ${currentEntry.episodeNumber}`} className="w-full rounded-3xl shadow-2xl object-contain max-h-[65vh]" draggable={false} />
              {/* Scroll decoration */}
              <div className="absolute -bottom-3 -right-3 w-16 h-16 rounded-full bg-indigo-500/20 backdrop-blur-md border border-indigo-500/30 flex items-center justify-center">
                <span className="material-symbols-outlined text-indigo-400 text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>history_edu</span>
              </div>
            </div>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1 z-10">
              {entries.slice(Math.max(0, currentIndex - 3), Math.min(entries.length, currentIndex + 4)).map((_, i) => {
                ...
                return <div key={idx} className={`rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-6 h-2 bg-indigo-400' : 'w-2 h-2 bg-white/30'}`} />;
              })}
            </div>
          </>
        ) : (
      ...
      {currentEntry && !showBottomSheet && (
        <div className="absolute bottom-8 right-6 z-20 flex items-center gap-3 animate-in slide-in-from-bottom-10 fade-in duration-300">
          ...
          <button onClick={() => { setShowBottomSheet(true); window.history.pushState({ popup: 'tangoHistorySheet' }, ''); }} className="relative w-14 h-14 bg-indigo-600/95 backdrop-blur-md rounded-full shadow-2xl flex items-center justify-center text-white hover:scale-105 active:scale-95 transition-transform border border-indigo-500/30">
            <span className="material-symbols-outlined text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>chat_bubble</span>
            <span className="absolute -top-1.5 -right-1.5 bg-indigo-500 text-white text-[11px] font-black min-w-[24px] h-6 px-1.5 flex items-center justify-center rounded-full border-2 border-indigo-700 shadow-sm">{reviews.length}</span>
          </button>
        </div>
      )}
                ...
                {reviews.length === 0 ? (
                  ...
                  <div key={review.id} className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0">
                      <span className="material-symbols-outlined text-indigo-400 text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>person</span>
                    </div>

...
function getSortedEvents(events: CoverEvent[]) {
  const mapped = events.map(ev => {
    ...
    // 서울 내부에서의 홍대/강남 정렬 가중치 부여 (홍대가 먼저 나오도록 함)
    let subWeight = 99;
    if (cityGroup === 'SEOUL') {
      const dLabel = getDistrictLabel(ev);
      if (dLabel === '서울 홍대') subWeight = 1;
      else if (dLabel === '서울 강남') subWeight = 2;
    }

    return { ...ev, groupWeight, subWeight, cityGroup };
  });

  mapped.sort((a, b) => {
    if (a.groupWeight !== b.groupWeight) return a.groupWeight - b.groupWeight;
    if (a.subWeight !== b.subWeight) return a.subWeight - b.subWeight;
...
/* ─── Class simplified text cell ─── */
function ClassCell({ label, items, colSpan = 'col-span-3' }: { label: string; items: CoverEvent[]; colSpan?: string }) {
  const groups: Record<string, CoverEvent[]> = {};
  items.forEach(ev => {
    const dLabel = getDistrictLabel(ev);
    if (!groups[dLabel]) groups[dLabel] = [];
    groups[dLabel].push(ev);
  });

  const orderedGroups: { label: string; list: CoverEvent[] }[] = [];
  items.forEach(ev => {
    const dLabel = getDistrictLabel(ev);
    if (!orderedGroups.some(g => g.label === dLabel)) {
      orderedGroups.push({ label: dLabel, list: groups[dLabel] });
    }
  });

  return (
    <div className={`${colSpan} rounded-[24px] bg-gradient-to-br from-slate-800 to-slate-900 px-8 py-7`}>
      <p className="text-[30px] font-black text-indigo-400 tracking-wider uppercase mb-5">{label}</p>
      {orderedGroups.length > 0 ? (
        <div className="flex flex-col gap-6">
          {orderedGroups.map(g => (
            <div key={g.label} className="space-y-4">
              <div className="flex items-center gap-3">
                <svg className="w-[26px] h-[26px] text-indigo-400 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
                </svg>
                <span className="text-[26px] font-black text-white/80 tracking-wide">{g.label}</span>
              ...
              <div className="flex flex-col gap-y-3 pl-8">
                {g.list.map(p => (
                  <div key={p.id} className="flex items-center gap-3 min-w-0">
                    ...
                    <span className="text-[28px] font-bold text-white truncate">{p.titleNative || p.title}</span>
                    <span className="text-[24px] text-white/50 flex-shrink-0">{p.startTime || ''}</span>
                    {p.instructor && <span className="text-[24px] text-orange-300 truncate">· {p.instructor}</span>}
                    {p.location && <span className="text-[24px] text-white/30 truncate">({p.location})</span>}
                  </div>
                ))}
              </div>
            </div>
        ...
        <p className="text-[14px] text-white/25">—</p>
      )}
    </div>
  );
}

export default function ThemeMagazineE({
  ...
  return (
    <div className="w-[1080px] bg-[#f5f5f7] flex flex-col text-gray-900" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>
      ...
      <div className="px-6 pb-6">
        ...
        {/* ── 클래스: 심플 텍스트 스타일 (이미지 없음) ── */}
        {hasClasses && (
          <div className="mt-8">
            <ClassCell label="📖 클래스" items={sortedClasses} colSpan="col-span-3" />
          </div>
        )}

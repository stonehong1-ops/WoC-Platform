...
function ImageCard({ ev, aspect = '3/4' }: { ev: CoverEvent; aspect?: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[3px]" style={{ aspectRatio: aspect }}>
      ...
      {ev.location && (
        <span className="absolute top-[3px] right-[3px] bg-black/50 text-white text-[4px] font-bold px-[3px] py-[1px] rounded-full leading-none truncate max-w-[50px]">
          {ev.location}
        </span>
      )}

      <div className="absolute bottom-0 left-0 right-0 p-[3px]">
        <p className="text-white font-black text-[6px] leading-[1.15]" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
          {ev.titleNative || ev.title}
        </p>
        {isMilonga && ev.organizer && (
          <p className="text-[4.5px] font-semibold text-white/60 truncate mt-[1px]">org {ev.organizer}</p>
        )}
        {isMilonga && ev.dj && (
          <p className="text-[4.5px] font-semibold text-white/60 truncate">dj {ev.dj}</p>
        )}
        {!isMilonga && ev.instructor && (
          <p className="text-[4.5px] font-semibold text-white/60 truncate mt-[1px]">{ev.instructor}</p>
        )}
      </div>
    </div>
  );
...
function PracticaCell({ label, items, maxShow = 5, colSpan = 'col-span-2' }: { label: string; items: CoverEvent[]; maxShow?: number; colSpan?: string }) {
  ...
  return (
    <div className={`${colSpan} rounded-[3px] bg-gradient-to-br from-slate-800 to-slate-900 px-[5px] py-[4px]`}>
...
export default function ThemeMagazineD({
  ...
  // Practica: 3-col grid → one cell gets col-span-2 if both exist, otherwise full span
  const bothPracticas = hongdaeP.length > 0 && gangnamP.length > 0;
    ...
    <div className="w-[1080px] bg-[#f5f5f7] flex flex-col" style={{ fontFamily: "'Inter', 'Noto Sans KR', sans-serif", minHeight: '1080px' }}>
      ...
      <div className="w-[360px] absolute top-0 left-0 origin-top-left scale-[3] flex flex-col text-gray-900 bg-[#f5f5f7]">
        ...
        <div className="px-[4px] pb-[4px]">
          ...
          {/* ── 밀롱가: 3-col grid ── */}
          {hasMilongas && (
            <div className="grid grid-cols-3 gap-[3px] items-end mt-[3px]">
          ...
          {/* ── 쁘락띠까: 2 text cells in 3-col grid ── */}
          {hasPracticas && (
            <div className="grid grid-cols-3 gap-[3px] mt-[4px]">
              {hongdaeP.length > 0 && (
                <PracticaCell label="🏃 쁘락띠까 · 홍대" items={hongdaeP} colSpan={bothPracticas ? '' : 'col-span-3'} />
              )}
              {gangnamP.length > 0 && (
                <PracticaCell label="🏃 쁘락띠까 · 강남" items={gangnamP} colSpan={bothPracticas ? 'col-span-2' : 'col-span-3'} />
              )}
            </div>
          )}
          ...
          {/* ── 클래스: 3-col grid ── */}
          {hasClasses && (
            <div className="grid grid-cols-3 gap-[3px] items-end mt-[4px]">

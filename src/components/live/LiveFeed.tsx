...
export default function LiveFeed({ entityType, entityId, userId, viewMode, className = '' }: LiveFeedProps) {
  ...
  const getDashboardHtml = () => {
    ...
    // 다국어 로케일 번역 치환 헬퍼 (영한 무결성 및 다국어 100% 연동)
    const tGroupStreams = (count: number) => {
      const translation = t('live-dashboard.group_streams');
      return translation ? translation.replace('{count}', String(count)) : `${count} Group Streams`;
    };
    const tMyMoments = (count: number) => {
      const translation = t('live-dashboard.my_moments');
      return translation ? translation.replace('{count}', String(count)) : `${count} My Moments`;
    };
    const tSocials = (count: number) => {
      const translation = t('live-dashboard.socials');
      return translation ? translation.replace('{count}', String(count)) : `${count} Socials`;
    };
    const tEvents = (count: number) => {
      const translation = t('live-dashboard.events');
      return translation ? translation.replace('{count}', String(count)) : `${count} Events`;
    };
    const tClasses = (count: number) => {
      const translation = t('live-dashboard.classes');
      return translation ? translation.replace('{count}', String(count)) : `${count} Classes`;
    };

    const titleText = 'Yesterday in TANGO SOCIETY'; // 슬로건이므로 로케일과 무관하게 상시 영어 원본 유지
    const subtitleText = t('live-dashboard.subtitle') || '수 많은 교육의 순간이 기록되었습니다.';

    // 2. 카운트 데이터 실시간 바인딩 및 다국어 치환
    html = html.replace('7 Group Streams', tGroupStreams(stats.groupCount));
    html = html.replace('3 My Moments', tMyMoments(stats.myCount));
    html = html.replace('3 Socials', tSocials(stats.socialCount));
    html = html.replace('0 Events', tEvents(stats.eventCount));
    html = html.replace('3 Classes', tClasses(stats.classCount));

    // 타이틀 및 부제목 로컬라이제이션 치환 (한글/영어 동적 대응)
    html = html.replace('Yesterday in TANGO SOCIETY', titleText);
    html = html.replace('수 많은 교육의 순간이 기록되었습니다.', subtitleText);

    // 2.5. 0% 및 진행률 링을 배제하고 즉시 활성화된 컴팩트 재생 아이콘으로 상시 고정 (Purge 완화 인라인 배경색 주입)
    const initialRingHtml = `
<div class="relative w-8 h-8 flex items-center justify-center rounded-full shrink-0" style="background-color: #004190;">
<span class="material-symbols-outlined text-[16px] font-bold text-white select-none">play_arrow</span>
    ...
    // 인라인 style에 직접 gradient 및 색상을 주입하여 테일윈드 Purge에 100% 무결하게 대비합니다!
    const inlineStyle = `opacity: ${initialOpacity}; cursor: ${initialCursor}; background: linear-gradient(to right, #0057bd, #004190); color: #ffffff;`;
    
    html = html.replace(
      '<button aria-label="Enter Live" class="',
      `<button aria-label="Enter Live" style="${inlineStyle}" class="${initialBounceClass} `
    );

    // DIVE IN 버튼 배경 그라디언트 대비 및 가독성 개선 (인라인 그라디언트를 이미 주입했으므로 기존 테일윈드 그라디언트 클래스는 제거)
    html = html.replace('from-primary-container to-primary', '');
  ...
  const handleBlockToggle = async (targetAuthorId: string) => {
    ...
    try {
      if (isTargetBlocked) {
        ...
        try {
          const autoReportId = `${user.uid}_block_${targetAuthorId}`;

...
const GroupProfileSetup = ({ group }: { group: Group }) => {
    ...
    <div className="flex-1 flex flex-col relative font-body selection:bg-blue-500/10">
      ...
      <AnimatePresence>
        {activePopup === 'home-config' && (
          <GroupHomeConfig onClose={() => setActivePopup(null)} />
        )}
        {activePopup === 'gallery' && (
          <GroupGalleryEditor onClose={() => setActivePopup(null)} />
        )}
        {activePopup === 'contact' && (
          <GroupContactEditor onClose={() => setActivePopup(null)} />
        )}
      </AnimatePresence>
    </div>
  );
...
export default GroupProfileSetup;

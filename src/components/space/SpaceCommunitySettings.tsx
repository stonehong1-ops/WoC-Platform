...
import SpaceBoardEditor from "./SpaceBoardEditor";
import SpaceRoleEditor from "./SpaceRoleEditor";

const SpaceCommunitySettings = ({ community }: { community: Community }) => {
  ...
  const settingsSteps = [
    {
      ...
      desc: "Define join strategies (Open, Approval, or Invite-only) for your community.",
    ...
    <div className="flex-1 flex flex-col relative bg-[#fcfdff] font-body selection:bg-blue-500/10 min-h-screen">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md flex justify-between items-center px-6 py-4 w-full border-b border-[#F0F2F9]">
        <div className="flex items-center gap-4">
          <h1 className="font-headline font-bold tracking-tight text-[#242c51] text-xl">Community Setting</h1>
        </div>
      ...
      <section className="px-6 md:px-10 max-w-6xl mx-auto w-full pt-10 pb-32">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div className="max-w-2xl">
            <h2 className="font-headline font-bold text-4xl md:text-5xl text-[#242c51] mb-5 leading-tight tracking-tight">Setup your community policy</h2>
            <p className="text-[#515981] text-lg leading-relaxed opacity-80">Complete the mandatory configurations below to define how your community interacts and operates.</p>
          </div>
        ...
        <div className="mt-16 p-8 rounded-[2rem] bg-gradient-to-br from-[#242c51] to-[#1a1f3d] text-white overflow-hidden relative group">
           ...
           <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
             <div className="flex items-center gap-5">
               ...
               <div>
                  <p className="text-lg font-bold">Need help with community settings?</p>
      ...
      <AnimatePresence>
        {activePopup === "membership" && (
          <SpaceMembershipEditor onClose={() => setActivePopup(null)} />
        )}
        {activePopup === "boards" && (
          <SpaceBoardEditor onClose={() => setActivePopup(null)} />
        )}
        {activePopup === "roles" && (
          <SpaceRoleEditor onClose={() => setActivePopup(null)} />
        )}
      </AnimatePresence>
    </div>
  );
...
export default SpaceCommunitySettings;

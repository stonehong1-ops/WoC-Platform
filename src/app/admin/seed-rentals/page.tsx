      ...
      try {
        ...
        for (const groupDoc of groupDocs.docs) {
          ...
          await updateDoc(doc(db, 'groups', groupId), {
            ...
            'rentalSettings': {
              currency: "KRW",
              rentalInfo: groupData.description || t('rental.default_desc'),
          ...
          if (existingSpaces.empty) {
            ...
            const rentalData = {
              groupId: groupId,
              title: (groupData.name || t('nav.class')) + ' ' + t('nav.rental'),
              description: groupData.description || t('rental.default_desc'),
              location: t('common.seoul'), 
              address: groupData.description?.split('위치한')?.[0]?.trim() || t('common.seoul'),
              category: 'Dance Studio',
              pricePerHour: 10000, // Using lowest tier as display base price
              minHours: 1,
              facilities: ['Wi-Fi', t('rental.facilities.mirror'), t('rental.air_con')],

...
export default function LostFoundDetailPage() {
  ...
  const handleBlockToggle = async () => {
    ...
    try {
      if (isBlocked) {
        ...
        try {
          const autoReportId = `${user.uid}_block_${item!.authorId}`;
  ...
  const handleChatWithAuthor = async () => {
    ...
    try {
      ...
      const itemInfo = `[${item.type === 'LOST' ? t('lost.lost') : t('lost.found')} ${t('lost.inquiry_title')}]\n${t('lost.register.label_title')}: ${item.title}\n${t('lost.register.label_location')}: ${item.location}\n${t('lost.inquiry_link')}: ${window.location.origin}/lost/${item.id}`;

...
const krDictionary: Record<string, string> = {
  'syncfit.title': 'SyncFit 실시간 협업 시스템 V1',
  ...
  'syncfit.scm.factoryCost': '원단 비용 (RMB/m)',
  'syncfit.scm.exchangeRate': '환율 (CNY/USD)',
  'syncfit.scm.duty': '공임 비용 (CMT)',
  'syncfit.scm.shipping': '소요량',
  ...
  // Stitch 최신 리소스 추가 번역어
  'syncfit.search.placeholder': '스타일 검색...',
  'syncfit.button.add_product': '새 제품 추가',
  'syncfit.scm.supply_chain_status': '공급망 상태',
  'syncfit.scm.production_timeline': '생산 타임라인',
  'syncfit.scm.ai_insights': 'AI 인사이트',
  'syncfit.scm.export_sheet': 'SCM 시트 내보내기',
  'syncfit.showroom.version_spec': '업데이트된 포켓 패턴 V2',
  'syncfit.showroom.version_desc': '업로드: Sarah J. • 2.4 MB',
};

const cnDictionary: Record<string, string> = {
  'syncfit.title': 'SyncFit 实时协作系统 V1',
  ...
  'syncfit.scm.factoryCost': '面料成本 (RMB/m)',
  'syncfit.scm.exchangeRate': '汇率 (CNY/USD)',
  'syncfit.scm.duty': '加工费 (CMT)',
  'syncfit.scm.shipping': '用量',
  ...
  // Stitch 최신 리소스 추가 번역어
  'syncfit.search.placeholder': '搜索款式...',
  'syncfit.button.add_product': '添加新产品',
  'syncfit.scm.supply_chain_status': '供应链状态',
  'syncfit.scm.production_timeline': '生产时间线',
  'syncfit.scm.ai_insights': 'AI 智能洞察',
  'syncfit.scm.export_sheet': '导出 SCM 表格',
  'syncfit.showroom.version_spec': '更新的口袋纸样 V2',
  'syncfit.showroom.version_desc': '上传: Sarah J. • 2.4 MB',
};
...
export function SyncFitLanguageProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    setIsMounted(true);
    const saved = localStorage.getItem('syncfit1_language') as Language;
  ...
  const setLanguage = useCallback((lang: Language) => {
    setLangState(lang);
    localStorage.setItem('syncfit1_language', lang);
  }, []);

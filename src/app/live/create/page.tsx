          ...
          {searchMode && (
            <div className="fixed inset-0 z-[200] flex items-end justify-center">
              ...
              <motion.div 
                ...
                className="relative w-full max-w-xl h-[85vh] bg-white rounded-t-[2rem] flex flex-col overflow-hidden shadow-2xl"
              >
                ...
                <div className="p-4 bg-gray-50 shrink-0">
                  <div className="relative">
                    <input
                      ...
                      autoFocus
                    />
                    {isSearching && <div className="absolute right-4 top-3.5 w-4 h-4 border-2 border-primary/30 border-t-primary rounded-full animate-spin" />}
                  </div>
                </div>
                  ...
                  {searchResults.length > 0 ? (
                    <div className="space-y-1">
                      {searchResults.map(r => (
                        <button
                          key={`${r.type}-${r.id}`}
                          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 rounded-xl text-left transition-colors"
                          onClick={() => addSearchResult(r)}
                        >
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${CLR[r.type] || 'bg-gray-100 text-gray-400 border-gray-200'}`}>
                            {ICON[r.type]}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-semibold text-gray-900 truncate">{r.name}</div>
                            <div className="text-xs text-gray-400 truncate">{r.type.toUpperCase()} · {r.subtitle}</div>
                          </div>
                        </button>
                      ))}
                    </div>

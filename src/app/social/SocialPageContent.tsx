...
export default function SocialPageContent() {
  ...
  useEffect(() => {
    ...
    const filterBar = (
      <div className="relative w-full bg-white flex items-center justify-between px-3.5 py-2 shadow-sm z-30 h-11">
        {/* 주차 네비게이션 (클래스 탭의 네비게이션 스타일 이식) */}
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setActiveWeek(prev => Math.max(1, prev - 1))} 
            disabled={activeWeek <= 1}
            className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-slate-100 active:scale-90 transition-all text-slate-500 disabled:opacity-30 disabled:pointer-events-none"
          >
            <span className="material-symbols-outlined text-[18px]">chevron_left</span>
          </button>
          <span className="text-[12px] font-bold text-slate-700 min-w-[50px] text-center">

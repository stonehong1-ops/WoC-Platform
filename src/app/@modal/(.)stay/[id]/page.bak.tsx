    ...
    if (!startDate || (startDate && endDate)) {
      ...
      if (isBefore(day, startDate)) {
        ...
        if (isRangeBlocked(startDate, day)) {
          alert('선택하신 기간 사이에 예약 불가능한 날짜가 포함되어 있습니다.');
          return;
        }
        setEndDate(day);
      }
            ...
            <div className="bg-surface-container-highest shadow-2xl rounded-3xl p-5 border border-outline-variant/30 flex items-center justify-between animate-in slide-in-from-bottom-8 duration-500 h-[96px]">
              ...
              {!startDate ? (
                ...
                <>
                  ...
                  <button 
                    onClick={clearDates} 
                    className="text-on-surface-variant text-label-sm font-label-sm hover:underline underline-offset-2 transition-all px-2"
                  >
                    Cancel
                  </button>
                </>
                ...
                <>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Link href={checkoutLink} onClick={onDismiss}>
                      <button className="bg-primary text-on-primary px-6 py-2.5 rounded-xl font-title-md shadow-md active:scale-95 transition-transform whitespace-nowrap">
                        Book Now
                      </button>
                    </Link>
                  </div>

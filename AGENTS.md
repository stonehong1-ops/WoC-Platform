...
- **배포 주체의 독점권**: 현재 20명의 대규모 인간 개발자 팀이 협업 중이더라도, 실제 프로덕션 서버로의 빌드 및 배포 작업(`npx vercel --prod` 등)은 오직 **"Agent Manager" (AI 에이전트)**만이 단독으로 수행합니다.
...
- **2.7.0 기준 버전 관리 프로토콜**:
  1. **마이너 세부 버전 규칙**: 프로젝트 기준 버전인 `2.7.0`을 기점으로, 후속 기능 개선이나 버그 패치는 세부 하위 버전(패치 버전)을 `.1`씩 안전하게 순차 향상합니다 (예: `2.7.0` → `2.7.1` → `2.7.2`).
  2. **메이저/마이너 버전 인상**: 부 버전(Minor: .x) 또는 주 버전(Major: x.)을 인상 및 변경하는 큰 업데이트는 반드시 기획자 **스톤님**의 명시적 변경 요청이 존재할 경우에만 유동적으로 수행합니다.

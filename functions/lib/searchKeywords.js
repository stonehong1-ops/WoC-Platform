...
function buildSearchKeywords(...sources) {
    ...
    for (const raw of sources) {
        ...
        if (!normalized)
            continue;
        const base = normalized.slice(0, exports.MAX_SOURCE_LENGTH);
        for (let start = 0; start < base.length; start++) {
            for (let len = 1; len <= exports.MAX_SUBSTRING_LENGTH && start + len <= base.length; len++) {

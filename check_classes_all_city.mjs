...
async function main() {
  ...
  for (let offset = 0; offset <= 2; offset++) {
    ...
    allClasses.forEach(cls => {
      ...
      (Array.isArray(cls.schedule) ? cls.schedule : (cls.schedule ? [{ date: cls.schedule.startDate, timeSlot: cls.schedule.startTime && cls.schedule.endTime ? (cls.schedule.startTime + ' - ' + cls.schedule.endTime) : '시간 조율' }] : [])).forEach(s => {
        if (!s.date) return;
    ...
    weekClasses.forEach(c => {
      if (c.groupId === "juni-yujin-tango") {
        console.log(`  - [${c.groupId}] ${c.title} | ��¥: ${c.scheduleEntry.date} | �ð�: ${c.scheduleEntry.timeSlot} | ���: ${c.location}`);
      }
    });
  }
}
